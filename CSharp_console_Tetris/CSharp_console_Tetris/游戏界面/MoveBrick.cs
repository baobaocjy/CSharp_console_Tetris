...
namespace CSharp_console_Tetris
{
    public class MoveBrick//搬砖工人
    {
        ...
        /// </summary>
        /// <returns></returns>
        public Position[] PrintPositions()
        {
            Position[] positions = new Position[4];
            for (int i = 0; i < 4; i++)
            {
                positions[i] = positionCentre + BlockDataStorage<Position>.GetData(brickType, rotation, i);
            }
            return positions;
        }
        //上移
        ...
        /// 反向移动方块
        /// </summary>
        public void MoveUp(ConsoleKey keyDirection)
        {
            ...
            switch (keyDirection) // 使用 key.Key 提取按键的值
            {
                ...
                case ConsoleKey.D: // 使用 ConsoleKey 枚举值进行判断
                    ...
                    break;
                case ConsoleKey.S: // 使用 ConsoleKey 枚举值进行判断
                    positionCentre += new Position(0, -1); // 上移
